...
`, filepath.Join(tmpdir, "root"), filepath.Join(tmpdir, "state"), address, filepath.Join(tmpdir, "debug.sock"))
	...
	if c.snapshotter != "" {
		...
		// Start snapshotter if using an external containerd snapshotter plugin
		if c.snapshotter == "stargz" || c.snapshotter == "fuse-overlayfs" {
			...
			var err error = nil
			var snCl func() error = nil

			switch c.snapshotter {
			case "stargz":
				snCl, err = c.runStargzSnapshotter(cfg, rootlessKitState, snPath, snRoot)
			case "fuse-overlayfs":
				snCl, err = c.runFuseOverlayfsSnapshotter(cfg, rootlessKitState, snPath, snRoot)
			}
	...
	// Start containerd
	ctdStop, err := c.runContainerdProcess(cfg, rootlessKitState, containerdArgs, address, c.extraEnv)
	if err != nil {
		return nil, nil, err
	}
...
func (c *containerd) runRootlesskit(tmpdir string, cfg *BackendConfig, deferF *multiCloser) (string, error) {
	...
	processStop, err := c.runContainerdProcess(cfg, "", args, filepath.Join(rootlessKitState, "api.sock"), nil)
	if err != nil {
		return "", err
	}
	deferF.append(processStop)
...
func (c *containerd) runContainerdProcess(cfg *BackendConfig, rootlessKitState string, args []string, unixSocketToWait string, extraEnv []string) (cl func() error, err error) {
	deferF := &multiCloser{}
	cl = deferF.F()
	...
	if err = waitUnix(unixSocketToWait, 10*time.Second, cmd); err != nil {
		snStop()
		return nil, errors.Wrapf(err, "%s did not start up: %s", cmd.Path, formatLogs(cfg.Logs))
	}
...
func (c *containerd) runStargzSnapshotter(cfg *BackendConfig, rootlessKitState string, snPath string, snRoot string) (func() error, error) {
	...
	return c.runContainerdProcess(cfg, rootlessKitState, args, snPath, nil)
}

func (c *containerd) runFuseOverlayfsSnapshotter(cfg *BackendConfig, rootlessKitState string, snPath string, snRoot string) (func() error, error) {
	binary := "containerd-fuse-overlayfs-grpc"
	if err := lookupBinary(binary); err != nil {
		return nil, err
	}

	args := []string{binary, snPath, snRoot}
